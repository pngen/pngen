...
| [Fabric Scheduler](https://github.com/pngen/Fabric-Scheduler) | Topology-aware, generation-bound workload placement across accelerators, CPUs, memory, NICs, storage, reservations, congestion, locality, communication cost, failure domains, and execution authority | Where should this work run now, given current capacity, topology, reservations, congestion, locality, communication cost, and policy — and which placement decision remains authoritative as infrastructure state changes? |
| [Communication Planner](https://github.com/pngen/Communication-Planner) | Topology-aware construction, validation, ranking, commitment, execution handoff, and revalidation of point-to-point, staged, relay, multicast, and collective communication plans across accelerators, CPUs, memory, NICs, storage, and distributed infrastructure | How should this data move from source to destination now, through which links and stages, under current topology, capacity, congestion, compatibility, and authority constraints — and which communication plan remains valid as infrastructure state changes? |
| [Collective Scheduler](https://github.com/pngen/Collective-Scheduler) | Temporal arbitration of competing collective communication plans across readiness, dependencies, priorities, deadlines, reservations, congestion, overlap, fairness, and generation-bound execution authority. | Which collective should run now, which must wait, which may safely overlap, and which schedule remains authoritative? |
| [GPU Memory Service](https://github.com/pngen/GPU-Memory-Service) | GPU memory allocation, residency, reclamation, eviction, compaction, pressure, ownership, contiguity, persistence, and generation-bound authority across heterogeneous accelerator infrastructure. | Which GPU-memory bytes are allocated, resident, reclaimable, movable, pinned, or free now, who owns them, and what action may safely change that state? |
| [Engine Residency](https://github.com/pngen/Engine-Residency) | Serving-engine incarnation, preparation, readiness, standby, activation, drain, replacement, recovery, and serving-use authority across heterogeneous accelerator infrastructure. | Which engine instance is prepared to serve this workload, what evidence makes it ready, who may authorize its use, and how does that authority survive replacement or restart? |
| [Failover Fabric](https://github.com/pngen/Failover-Fabric) | Service failover, fencing, promotion, routing, request ambiguity, state recovery, failback, and replacement authority across distributed AI infrastructure. | When a serving target fails, which replacement may take over, what makes that transition valid, and how does the system prevent stale workers, routes, results, or recovered state from becoming current? |
| [Recovery Planner](https://github.com/pngen/Recovery-Planner) | Authoritative recovery strategy selection across restore, restart, migrate, rehydrate, shadow promotion, failover, and recomputation using state integrity, compatibility, readiness, resources, topology, preserved progress, latency, deadline, cost, risk, confidence, freshness, and execution authority. | When execution fails, which recovery path is valid now, which best satisfies the objective, and what evidence makes that decision authoritative? |
| [SLO Fabric](https://github.com/pngen/SLO-Fabric) | Service-level objective contracts, evidence ingestion, compliance evaluation, error budgets, binding-constraint identification, enforcement authorization, re-evaluation, and durable SLO state across latency, availability, throughput, recovery time, memory pressure, and cost. | Which service-level objectives are currently satisfied or violated, which constraint is binding, what error budget remains, and which enforcement action is still authoritative under current evidence? |
| [Tail Governor](https://github.com/pngen/Tail-Governor) | Tail-latency governance across exact p95/p99/p999 quantiles, completed-work evidence, cause decomposition, intervention feasibility/economics, bounded authority, hysteresis/cooldown, and post-action verification. | Which tail-latency condition is currently authoritative, what is causing it, which intervention is valid and economically justified, and did that intervention actually improve the tail under fresh evidence? |
| [Cost Governor](https://github.com/pngen/Cost-Governor) | Execution economics across accelerator time, energy, transfer, memory, residency, retries, recovery, and per-request/per-token budgets. | What will execution cost under current resource state and authority, which plan remains legal under hard budgets and service constraints, and when must execution be rejected, deferred, replanned, or stopped? |
| [Efficiency Ledger](https://github.com/pngen/Efficiency-Ledger) | Attribution of useful work, necessary overhead, avoidable waste, avoided work, and stranded capacity across accelerator execution. | Of the resources consumed or reserved by a workload, what actually contributed to authoritative useful output, what was necessary overhead, what was avoidable waste, what work was avoided through reuse, and where did capacity become stranded? |
| [Utilization Observatory](https://github.com/pngen/Utilization-Observatory) | Accelerator-capacity observability across compute busy/idle, useful vs non-useful work, waiting, transfers, reservations, residency, fragmentation, fencing, unavailable capacity, memory occupancy, freshness, provenance, and historical timelines. | How much capacity is actively executing, waiting, transferring, reserved, resident, fragmented, fenced, unavailable, or unknown — and how much of the busy time is actually useful? |
...
| [Contention Governor](https://github.com/pngen/Contention-Governor) | Active cross-workload accelerator contention control using interference evidence, policy, SLOs, workload value, fairness, cost constraints, deterministic intervention ranking, and generation-fenced enforcement. | Once interference is established, which workload should yield, by how much, through which legal intervention, under whose authority, and how do we prove the action actually improved the system rather than simply moving the problem elsewhere? |
...
| [DPU Fabric](https://github.com/pngen/DPU-Fabric) | Programmable infrastructure offload across DPUs, SmartNICs, infrastructure processors, and adjacent accelerator infrastructure, including capability, isolation, locality, lifecycle, resource, and generation-bound authority. | Which infrastructure function may be offloaded to which programmable device now — under what capability, isolation, locality, lifecycle, resource, and generation-bound authority? |
