...
| 2 | [Context Fabric](https://github.com/pngen/Context-Fabric) | Identity, ownership, generations, replicas, persistence, migration, lineage, and placement for reusable computational state. | Where does accumulated reusable computation live? |
| 3 | [Compute Fabric](https://github.com/pngen/Compute-Fabric) | Distributed execution placement across heterogeneous compute nodes. | Where should the next computation run? |
...
| 10 | [Topology Fabric](https://github.com/pngen/Topology-Fabric) | Hardware and interconnect discovery, topology, locality, path cost, and candidate ranking. | What hardware topology exists, what connects it, and what does that imply about locality and path cost? |
| 11 | [Memory Pressure](https://github.com/pngen/Memory-Pressure) | Memory scarcity detection, modeling, pressure state, budgets, thresholds, and governed response. | When memory becomes scarce, how should the system know and respond before scarcity becomes failure? |
| 12 | [Prefix Fabric](https://github.com/pngen/Prefix-Fabric) | Reusable token-prefix identity, indexing, matching, lineage, segmentation, and reuse discovery. | What reusable prefix structure already exists, which requests overlap with it, and what exact relationship justifies reuse? |
...
| 19 | [Decode Fabric](https://github.com/pngen/Decode-Fabric) | Continuous scheduling, regrouping, execution, and governance of iterative token decoding. | How should active generation work be scheduled, regrouped, executed, and governed token by token so many concurrent sequences make progress without sacrificing latency, fairness, memory headroom, cancellation responsiveness, or throughput? |
| 20 | [Speculation Fabric](https://github.com/pngen/Speculation-Fabric) | Speculative proposal, verification, partial acceptance, rollback, branching, retry, and distributed authority. | How should speculative generation be proposed, verified, accepted, rejected, rolled back, and accounted for so work can run ahead of authority without sacrificing correctness, fairness, memory discipline, or deterministic recovery? |
| 21 | [Kernel Cache](https://github.com/pngen/Kernel-Cache) | Caching, validating, persisting, reusing, invalidating, rebuilding, and evicting compiled AI kernel artifacts. | Which compiled kernel artifact may be reused, under what compatibility constraints, where should it reside, and when must it be invalidated, rebuilt, or evicted? |
| 22 | [Graph Cache](https://github.com/pngen/Graph-Cache) | Caching, validating, persisting, reusing, invalidating, and recapturing AI execution graphs. | Which captured execution graph may be safely replayed for this workload, under what compatibility and generation constraints, and when has that graph become stale? |
| 23 | [Compilation Fabric](https://github.com/pngen/Compilation-Fabric) | AI compilation specialization, code generation, validation, reuse, invalidation, reproducibility, and accelerator-targeted deployment. | How should executable AI artifacts be derived, specialized, validated, reproduced, invalidated, and deployed across heterogeneous accelerators? |
| 24 | [Placement Observatory](https://github.com/pngen/Placement-Observatory) | Reconstructing, explaining, replaying, and comparing AI workload placement decisions across compute, memory, topology, queue, capacity, and locality constraints. | Why did this work land here, what alternatives existed, which constraints and costs drove the decision, and can that decision be reconstructed later from evidence rather than guessed from outcome? |
| 25 | [Latency Governor](https://github.com/pngen/Latency-Governor) | Enforcing explicit latency SLOs across admission, waiting, batching, placement, movement, execution, speculation, retry, and recovery. | How should resource and execution decisions be governed against explicit latency obligations before the deadline is lost? |
...
| 57 | [Preemption Fabric](https://github.com/pngen/Preemption-Fabric) | Safe interruption, state preservation, resource release, and authoritative resume of accelerator work. | Can this work be interrupted safely now, and under what conditions may it resume? |
| 58 | [Reservation Fabric](https://github.com/pngen/Reservation-Fabric) | Advance reservation of compute, accelerator memory, host memory, bandwidth, residency, and network capacity. | What future capacity has been promised, and can that promise still be honored? |
| 59 | [Capacity Fabric](https://github.com/pngen/Capacity-Fabric) | Current, generation-aware accelerator capacity modeling across devices, memory, topology, reservations, fragmentation, and execution authority. | Can this infrastructure actually fit and execute the requested work now? |
| 60 | [Fragmentation Governor](https://github.com/pngen/Fragmentation-Governor) | Accelerator-memory fragmentation detection, fit analysis, remediation planning, revalidation, and execution authority. | When enough memory exists in aggregate but useful work still cannot fit, what fragmentation is present and what remediation is justified? |
| 61 | [Congestion Fabric](https://github.com/pngen/Congestion-Fabric) | Live congestion, queue/backlog state, residual bandwidth, bottleneck attribution, fairness, backpressure, and generation-bound traffic authority across accelerator, host-memory, PCIe, network, storage, and collective domains | Where is infrastructure traffic congested now, which flows and resource generations are causing it, how much useful capacity remains, and what bounded action should callers take? |
| 62 | [Fabric Scheduler](https://github.com/pngen/Fabric-Scheduler) | Topology-aware, generation-bound workload placement across accelerators, CPUs, memory, NICs, storage, reservations, congestion, locality, communication cost, failure domains, and execution authority | Where should this work run now, given current capacity, topology, reservations, congestion, locality, communication cost, and policy — and which placement decision remains authoritative as infrastructure state changes? |
| 63 | [Communication Planner](https://github.com/pngen/Communication-Planner) | Topology-aware construction, validation, ranking, commitment, execution handoff, and revalidation of point-to-point, staged, relay, multicast, and collective communication plans across accelerators, CPUs, memory, NICs, storage, and distributed infrastructure | How should this data move from source to destination now, through which links and stages, under current topology, capacity, congestion, compatibility, and authority constraints — and which communication plan remains valid as infrastructure state changes? |
| 64 | [Collective Scheduler](https://github.com/pngen/Collective-Scheduler) | Temporal arbitration of competing collective communication plans across readiness, dependencies, priorities, deadlines, reservations, congestion, overlap, fairness, and generation-bound execution authority. | Which collective should run now, which must wait, which may safely overlap, and which schedule remains authoritative? |
| 65 | [GPU Memory Service](https://github.com/pngen/GPU-Memory-Service) | GPU memory allocation, residency, reclamation, eviction, compaction, pressure, ownership, contiguity, persistence, and generation-bound authority across heterogeneous accelerator infrastructure. | Which GPU-memory bytes are allocated, resident, reclaimable, movable, pinned, or free now, who owns them, and what action may safely change that state? |
| 66 | [Engine Residency](https://github.com/pngen/Engine-Residency) | Serving-engine incarnation, preparation, readiness, standby, activation, drain, replacement, recovery, and serving-use authority across heterogeneous accelerator infrastructure. | Which engine instance is prepared to serve this workload, what evidence makes it ready, who may authorize its use, and how does that authority survive replacement or restart? |
| 67 | [Failover Fabric](https://github.com/pngen/Failover-Fabric) | Service failover, fencing, promotion, routing, request ambiguity, state recovery, failback, and replacement authority across distributed AI infrastructure. | When a serving target fails, which replacement may take over, what makes that transition valid, and how does the system prevent stale workers, routes, results, or recovered state from becoming current? |
| 68 | [Recovery Planner](https://github.com/pngen/Recovery-Planner) | Authoritative recovery strategy selection across restore, restart, migrate, rehydrate, shadow promotion, failover, and recomputation using state integrity, compatibility, readiness, resources, topology, preserved progress, latency, deadline, cost, risk, confidence, freshness, and execution authority. | When execution fails, which recovery path is valid now, which best satisfies the objective, and what evidence makes that decision authoritative? |
| 69 | [SLO Fabric](https://github.com/pngen/SLO-Fabric) | Service-level objective contracts, evidence ingestion, compliance evaluation, error budgets, binding-constraint identification, enforcement authorization, re-evaluation, and durable SLO state across latency, availability, throughput, recovery time, memory pressure, and cost. | Which service-level objectives are currently satisfied or violated, which constraint is binding, what error budget remains, and which enforcement action is still authoritative under current evidence? |
| 70 | [Tail Governor](https://github.com/pngen/Tail-Governor) | Tail-latency governance across exact p95/p99/p999 quantiles, completed-work evidence, cause decomposition, intervention feasibility/economics, bounded authority, hysteresis/cooldown, and post-action verification. | Which tail-latency condition is currently authoritative, what is causing it, which intervention is valid and economically justified, and did that intervention actually improve the tail under fresh evidence? |
| 71 | [Cost Governor](https://github.com/pngen/Cost-Governor) | Execution economics across accelerator time, energy, transfer, memory, residency, retries, recovery, and per-request/per-token budgets. | What will execution cost under current resource state and authority, which plan remains legal under hard budgets and service constraints, and when must execution be rejected, deferred, replanned, or stopped? |
| 72 | [Efficiency Ledger](https://github.com/pngen/Efficiency-Ledger) | Attribution of useful work, necessary overhead, avoidable waste, avoided work, and stranded capacity across accelerator execution. | Of the resources consumed or reserved by a workload, what actually contributed to authoritative useful output, what was necessary overhead, what was avoidable waste, what work was avoided through reuse, and where did capacity become stranded? |
